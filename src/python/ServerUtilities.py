...
TRANSFERDB_STATES = {0: "NEW",
                     ...
                     6: "KILL",
                     7: "KILLED"}
TRANSFERDB_STATUSES = dict((v, k) for k, v in list(TRANSFERDB_STATES.items()))
...
PUBLICATIONDB_STATES = {0: "NEW",
                        ...
                        4: "RETRY",
                        5: "NOT_REQUIRED"}
PUBLICATIONDB_STATUSES = dict((v, k) for k, v in list(PUBLICATIONDB_STATES.items()))
...
USER_ALLOWED_TRANSFERDB_STATES = {0: "NEW",
                                  3: "DONE"}
USER_ALLOWED_TRANSFERDB_STATUSES = dict((v, k) for k, v in list(TRANSFERDB_STATES.items()))

USER_ALLOWED_PUBLICATIONDB_STATES = {0: "NEW",
                                     4: "RETRY",
                                     5: "NOT_REQUIRED"}
USER_ALLOWED_PUBLICATIONDB_STATUSES = dict((v, k) for k, v in list(PUBLICATIONDB_STATES.items()))
...
def encodeRequest(configreq, listParams=None):
    ...
    if isinstance(configreq, dict):
        ...
        for k, v in list(configreq.items()):
